...
contract CryptoDevsDAO is Ownable {
    ...
    struct Proposal {
        uint256 nftTokenId;
        uint256 deadline; //untill proposal is active, proposal will be executed after this
        uint256 yayVotes;
        uint256 nayVotes;
        bool executed;
        mapping(uint256 => bool) voters; //tokenId to bool, checks nftId voted or not
    }
    ...
    function createProposal(uint256 _nftTokenId) external nftHolderOnly returns (uint256) {
        require(nftMarketplace.available(_nftTokenId), "NFT_NOT_FOR_SALE");// that particular nft should be available for sale
        Proposal storage proposal = proposals[numProposals];
        proposal.nftTokenId = _nftTokenId;

        proposal.deadline = block.timestamp + 5 days;
    ...
    //buy nft from marketplace when yay>nay
